...
!function() {
    ...
    if (!e.Filter) {
        ...
        t.prototype.filter = function(e, t) {
            ...
            for (n = 0; (c || d) && n < s; ++n) {
                ...
                m = i - c, c && c--;
                C = a[0] * t[n];
                for (S = 1; S < m; ++S) C += a[S] * t[n - S];
                for (;S < i; ++S) C += a[S] * u[S - m];
                for (m = r - d, d && d--, S = 0; S < m; ++S) C -= o[S] * e[n - 1 - S];
                for (;S < r; ++S) C -= o[S] * l[S - m];
                e[n] = C;
            }
            if (3 == i && 2 == r) for (var h, p, k = a[0], g = a[1], v = a[2], y = o[0], b = o[1], w = t[n - 1], A = t[n - 2], N = e[n - 1], M = e[n - 2]; n < s; ++n) h = A, 
            A = w, p = M, N = k * (w = t[n]) + g * A + v * h - y * (M = N) - b * p, e[n] = N; else for (;n < s; ++n) {
                var S, C = a[0] * t[n];
                for (S = 1; S < i; ++S) C += a[S] * t[n - S];
                for (S = 0; S < r; ++S) C -= o[S] * e[n - 1 - S];
                e[n] = C;
            }
            var I = Math.min(i - 1, s);
            for (n = i - 2; n >= I; --n) u[n] = u[n - I];
            for (n = 0; n < I; ++n) u[n] = t[s - 1 - n];
            for (I = Math.min(r, f), n = r - 1; n >= I; --n) l[n] = l[n - I];
    ...
    if (!e.FFT) {
        var t = function(e, t, n, o, r, a, i) {
            ...
            for (var l, u, s, f, c, d, m, h, p, k, g, v, y, b, w, A, N = 0, M = 0, S = 2 * o, C = n, I = n + i, x = n + 2 * i, L = a[o * i], D = .5773502691896258, F = C + i; C < F; ) p = e[C] * D, 
            k = t[C] * D, g = e[I] * D, v = t[I] * D, y = e[x] * D, b = t[x] * D, w = r[N], 
            f = g * (A = a[N]) + v * w, l = (s = g * w - v * A) - (c = y * (w = r[M]) - b * (A = a[M])), 
            u = f - (d = y * A + b * w), g = p - .5 * (m = s + c), v = k - .5 * (h = f + d), 
            l *= L, u *= L, e[C] = p + m, t[C] = k + h, e[x] = g + u, t[x] = v - l, e[I] = g - u, 
            t[I] = v + l, N += o, M += S, ++C, ++I, ++x;
        }, o = function(e, t, n, o, r, a, i, l) {
            for (var u, s, f, c, d, m, h, p, k, g, v, y, b, w, A, N, M, S, C, I, x, L, D = 0, F = 0, P = 0, O = 2 * o, T = 3 * o, B = n, R = n + i, E = n + 2 * i, U = n + 3 * i, V = B + i; B < V; ) b = .5 * e[B], 
            w = .5 * t[B], A = .5 * e[R], N = .5 * t[R], M = .5 * e[E], S = .5 * t[E], C = .5 * e[U], 
            I = .5 * t[U], u = A * (x = r[D]) - N * (L = a[D]), s = A * L + N * x, v = b - (f = M * (x = r[F]) - S * (L = a[F])), 
            y = w - (c = M * L + S * x), b += f, w += c, h = u + (d = C * (x = r[P]) - I * (L = a[P])), 
            p = s + (m = C * L + I * x), k = u - d, g = s - m, e[E] = b - h, t[E] = w - p, e[B] = b + h, 
            t[B] = w + p, l ? (e[R] = v - g, t[R] = y + k, e[U] = v + g, t[U] = y - k) : (e[R] = v + g, 
            t[R] = y - k, e[U] = v - g, t[U] = y + k), D += o, F += O, P += T, ++B, ++R, ++E, 
            ++U;
        }, r = function(e, t, n, o, r, a, i) {
            for (var l, u, s, f, c, d, m, h, p, k, g, v, y, b, w, A, N, M, S, C, I, x, L, D, F, P, O, T, B, R, E, U, V, G, q, _, W, z, j = 0, H = 0, K = 0, J = 0, Q = 2 * o, X = 3 * o, Y = 4 * o, $ = n, Z = n + i, ee = n + 2 * i, te = n + 3 * i, ne = n + 4 * i, oe = r[o * i], re = a[o * i], ae = r[2 * o * i], ie = a[2 * o * i], le = .4472135954999579, ue = $ + i; $ < ue; ) O = e[$] * le, 
            T = t[$] * le, B = e[Z] * le, R = t[Z] * le, E = e[ee] * le, U = t[ee] * le, V = e[te] * le, 
            G = t[te] * le, q = e[ne] * le, _ = t[ne] * le, l = O, u = T, s = B * (W = r[j]) - R * (z = a[j]), 
            f = B * z + R * W, c = E * (W = r[H]) - U * (z = a[H]), d = E * z + U * W, m = V * (W = r[K]) - G * (z = a[K]), 
            h = V * z + G * W, w = s + (p = q * (W = r[J]) - _ * (z = a[J])), A = f + (k = q * z + _ * W), 
            I = s - p, x = f - k, N = c + m, M = d + h, S = c - m, C = d - h, e[$] = O + w + N, 
            t[$] = T + A + M, g = l + w * oe + N * ae, v = u + A * oe + M * ae, y = x * re + C * ie, 
            b = -I * re - S * ie, e[Z] = g - y, t[Z] = v - b, e[ne] = g + y, t[ne] = v + b, 
            L = l + w * ae + N * oe, D = u + A * ae + M * oe, F = -x * ie + C * re, P = I * ie - S * re, 
            ...
            for (s = 0; s < i; ++s) {
                ...
                for (f = 0; f < l; ++f) {
                    m = y[0], h = b[0];
                    var N = 0;
                    for (c = 1; c < l; ++c) (N += A) >= u && (N -= u), p = y[c], k = b[c], m += p * (g = r[N]) - k * (v = a[N]), 
                    h += p * v + k * g;
                    e[d] = m, t[d] = h, d += i, A += o;
                }
            }
        }, i = function(e, l, u, s, f, c, d, m, h, p, k, g, v, y) {
            var b = h[p++], w = h[p++], A = u, N = u + b * w, M = d * m;
            if (1 == w) do {
                e[u] = s[c], l[u] = f[c], c += M, ++u;
            } while (u != N); else do {
                i(e, l, u, s, f, c, d * b, m, h, p, k, g, v, y), c += M, u += w;
            } while (u != N);
            switch (u = A, b) {
              case 2:
                t(e, l, u, d, k, g, w);
...
fluid.defaults("flock.midi.system", {
    ...
    var t = e.data, n = t.length, o = new Uint8Array(t);
    return 240 !== t[0] && 247 !== t[n - 1] ? ((o = new Uint8Array(n + 2))[0] = 240, 
    o[n + 1] = 247, o.set(t, 1)) : 240 !== t[0] ? ((o = new Uint8Array(n + 1))[0] = 240, 
    o.set(t, 1)) : 247 !== t[n - 1] && ((o = new Uint8Array(n + 1))[n] = 247, o.set(t, 0)), 
    o;
