...
(function () {
    ...
    flock.test.audioFile.testTriangleBuffer = function (decoded, sampleRate) {
        ...
        equal(format.sampleRate, sampleRate || 44100,
        ...
        // TODO: Fix this failure at non-44.1 KHz sample rates.
        flock.test.arrayEqualBothRounded(1, buffer, expected,
            "THIS TEST WILL FAIL AT SAMPLE RATES >44.1KHz: " +
            ...
            return function () {
                flock.audio.decode({
                    ...
                    success: function (decoded) {
                        flock.test.audioFile.testTriangleBuffer(decoded, config.sampleRate);
                        start();
                    }
                });
            };
